...
async def start_ai(
    ...
    container = _container(domain_name)
    ...
    try:
        # Write/refresh agent files and activate selected agent
        await write_all_agent_files(container, domain_name)
        await activate_agent(container, agent)
        ...
        # Start OpenCode
        await _start_opencode(container, token, env)

        # Wait for readiness
        ready = await _wait_ready(container)
        if not ready:
...
async def stop_ai(
    ...
    # Kill opencode process in container
    loop = asyncio.get_running_loop()
    def _kill():
        subprocess.run(
...
async def ai_ws(
    ...
    # ── Connect to opencode in container ──────────────────────────────────────
    container = _container(domain_name)
    # opencode serve listens on container-internal localhost:7878
    # We reach it via docker exec port-forward or via container network
    # Using the container's bridge IP / DNS name
...
async def opencode_auth_login(
    ...
    Runs `opencode auth login` and returns the URL for the user to visit.
    """
    loop = asyncio.get_running_loop()
    ...
    try:
        ...
        raise HTTPException(503, "OpenCode is not installed on this server.")
    except Exception as e:
