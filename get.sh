...
# Usage: curl -sSL https://raw.githubusercontent.com/timehub-uk/GnuKontrolR/master/get.sh | sudo bash
#   or:  sudo bash get.sh
# ─────────────────────────────────────────────────────────────────────────────
...
# ─── Root check ──────────────────────────────────────────────────────────────
[[ $EUID -eq 0 ]] || die "Run as root: sudo bash get.sh"
...
if [[ -f .env ]]; then
  ...
  # Interactive prompts — read from /dev/tty so they work when piped from curl
  exec 3</dev/tty
  read -rp "  Panel domain (e.g. panel.example.com): " PANEL_DOMAIN <&3
  read -rp "  Mail hostname (e.g. mail.example.com): " MAIL_HOST <&3
  read -rp "  Let's Encrypt email: " ACME_EMAIL <&3
  read -rp "  Timezone [UTC]: " TZ_INPUT <&3
  exec 3<&-
  TZ_INPUT="${TZ_INPUT:-UTC}"

  cat > .env <<ENVEOF
# Generated by get.sh on $(date -u +"%Y-%m-%d %H:%M UTC")
...
exec 3</dev/tty
read -rp "  Admin username [admin]: " ADMIN_USER <&3
ADMIN_USER="${ADMIN_USER:-admin}"
read -rp "  Admin email: " ADMIN_EMAIL <&3
read -rsp "  Admin password: " ADMIN_PASS <&3; echo
read -rsp "  Confirm password: " ADMIN_PASS2 <&3; echo
exec 3<&-
